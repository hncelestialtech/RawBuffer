...
struct rawbuf_proto{
    ...
    void reserve(size_t acapacity) {
        ...
        this->data_ptr =  (char*)rbrealloc(this->data_ptr, acapacity);
        this->data_capacity = acapacity;
    }
    ...
    RAWBUF_T* append(const RAWBUF_T* src, RAWBUF_M* offset_pointer){
        ...
        *offset_pointer = (RAWBUF_M)offset_diff;
        ...
        if(this->data_size > this->data_capacity ){
            ...
            this->data_ptr = (char*)rbrealloc(this->data_ptr, new_capacity);
        }
        if(src != 0){
...
struct rawbuf_builder : public rawbuf_writer_proto, public rawbuf_writer<RAWBUF_T>{
    ...
    rawbuf_builder(rawbuf_uint32 init_capcitya = init_capacity){
        this->data_ptr = (char*)rbmalloc(init_capcitya);
    ...
    rawbuf_builder(const RAWBUF_T &src, size_t src_size = 0){
        ...
        this->data_ptr = (char*)rbmalloc(src_size);
        this->data_size = sizeof(RAWBUF_T);
    ...
    rawbuf_builder& operator = (const RAWBUF_T &src){
        rbfree(this->data_ptr);
        this->data_ptr = (char*)rbmalloc(init_capacity);
        this->data_size = sizeof(RAWBUF_T);
